...
function Updatedocumentaionlinks() {
  ...
  useEffect(() => {
    const findData = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/get/getsubject/${id}`, {
          method: "GET",
          headers: {
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/update/subjects/${id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDocLink = async (id, url, urlIndex) => {
    try {
      const res = await fetch(`http://localhost:3000/api/delete/doclink/${id}/${urlIndex}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setSubjectData({
        ...subjectData,
        documentationlink: subjectData.documentationlink.filter((_, index) => index !== urlIndex),
      });
      ...
      <div className="flex flex-col bg-black">
        ...
        <div className="flex justify-between gap-3 sm:gap-5 md:gap-10 "> 
          ...
          <h1 className="text-center text-3xl">
            ...
            <span className="text-xs opacity-[0.4]"> Update Documentation Link</span>
          </h1>
            </div>
        </div>
          ...
          <div className="flex gap-10 items-center justify-center p-3">
            <label htmlFor="documentationlinkdesc" className="text-xs sm:text-lg">
          ...
          {/* <div className="flex flex-col items-center justify-center gap-4">
            {subjectData.documentationlink.length > 0 ? (
              subjectData.documentationlink.map((url, index) => (
                <div key={index} className="flex items-center gap-4 p-2">
                  ...
                  <button
                    onClick={() => handleDocLink(id, url, index)}
              ...
              <p className="text-center text-xs sm:text-sm">No documentation links available.</p>
            )}
          </div> */}   
          <div className="flex flex-col">
              ...
              <div key={urlIndex} className="flex items-center justify-center gap-5">
                ...
                <div className="text-red-600 flex flex-col gap-2">
                  <button
                    ...
                    onClick={() => handleDocLink(subjectData._id, url, urlIndex)}
                  >
                    <MdDelete size={24} />
                  </button>
                </div>
            ...
            {errorMessage && <p className="text-red-700 text-xs text-center">{errorMessage}</p>}
            {successMessage && <p className="text-green-500 text-xs text-center">{successMessage}</p>}
          </div> 
...
export default Updatedocumentaionlinks;
